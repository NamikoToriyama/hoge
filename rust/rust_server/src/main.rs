...
#[actix_web::main] // マクロらしい
async fn main() -> std::io::Result<()>{
    env::set_var("RUST_LOG", "actix_web=debug,actix_redis=info");//ログレベル
    env_logger::init();

    let host = env::var("HOST").unwrap_or_else(|_| "0.0.0.0".to_string()); // ホスト
    let port = env::var("PORT").unwrap_or_else(|_| "8000".to_string()); // ポート
